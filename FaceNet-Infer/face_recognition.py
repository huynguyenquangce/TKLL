...
# Khởi tạo biến cờ để kiểm tra xem lời chào đã được phát hay chưa
...
if __name__ == "__main__":
    ...
    embeddings, names = load_faceslist()  #danh sách bộ dữ liệu và tên đã được load
    # biến check xem đã chào người đó chưa
    check = np.full(shape=(len(names),), fill_value=False, dtype=bool)
    while cap.isOpened():
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if idx != -1:
                        ...
                        if not check[idx]:
                            text = f"Hello{names[idx]}, good morning. Hope you have a good day."
                            engine.say(text)
                            engine.runAndWait()
                            check[idx] = True
                    else:
                        ...
                        text_none= f"Good morning. Hope you have a good day."
                        engine.say(text_none)
                        engine.runAndWait()
